...
class ReservaTuSpot:
    ...
    def __init__(self, user_name, password, players, day, hour):
    ...
    def get(self, url: str, payload: dict = None):
        return requests.get(
            self.url + url, data=payload, headers={"Authorization": self.token}
        )
    ...
    def get_token(self):
        response = self.post(
            url="/user/login",
            payload={"usuario": self.user_name, "password": self.password},
        )
        self.token = response.json().get("Token")

    def schedules_availables(self, match_type: str, field="undefined"):
        return self.get(
            url=f"/reservas/bringHoursFree/{self.day}/{field}/{match_type}"
        ).json()

    def get_user_id(self, user_name: str):
        return str(
            self.get(url=f"/reservas/getID/{user_name}")
            .json()
            .get("resultado")[0]
            .get("ID_Usuario")
        )
    ...
    def closest_schedule(self, match_type: str):
        ...
        for available in response.get("resultado"):
            ...
            minutes = (
                int(minutes_splitted[1]) / self.hourly_lag
                if len(minutes_splitted) == 2
                else 0
            )
            wanted_hour, wanted_minutes = self.hour.split(".")
            if (
                hour == int(wanted_hour)
                and minutes == int(wanted_minutes)
                or hour > int(wanted_hour)
            ):
                return available
    ...
    def book(self):
        ...
        print(
            self.get(
                url=f"/reservas/makeReserva/{self_id}/1/{day}/{schedule}/{match_type}/{players}"
            ).json()
        )
